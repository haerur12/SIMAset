...
if(!$data) {
    header("Location: ruangan.php?action=notfound");
    exit;
}
...
if(isset($_GET['hapus'])) {
    ...
    if(mysqli_stmt_execute($stmt)) {
        ...
        header("Location: edit_ruangan.php?id=$id&error=delete_failed");
        exit;
    }
}
...
if(isset($_POST['update'])) {
    ...
    // Check duplicate kode (exclude current ID)
    $check = mysqli_query($conn, "SELECT id FROM ruangan WHERE kode_ruangan = '$kode_ruangan' AND id != $id");
    if(mysqli_num_rows($check) > 0) {
        header("Location: edit_ruangan.php?id=$id&error=duplicate");
        exit;
    }
    ...
    if(mysqli_query($conn, $query)) {
        header("Location: ruangan.php?action=updated");
        exit;
    } else {
        header("Location: edit_ruangan.php?id=$id&error=db");
        exit;
    }
}
...
if(isset($_GET['error'])) {
    if($_GET['error'] === 'duplicate') {
        $error_msg = 'Kode ruangan sudah digunakan oleh ruangan lain!';
        $error_type = 'duplicate';
    } elseif($_GET['error'] === 'db') {
        $error_msg = 'Terjadi kesalahan database. Silakan coba lagi.';
        $error_type = 'db';
    } elseif($_GET['error'] === 'delete_failed') {
        $error_msg = 'Gagal menghapus ruangan. Silakan coba lagi.';
        $error_type = 'delete';
    }
}

// Format tanggal
$created_at = isset($data['created_at']) ? date('d M Y, H:i', strtotime($data['created_at'])) : '-';
$updated_at = isset($data['updated_at']) && $data['updated_at'] ? date('d M Y, H:i', strtotime($data['updated_at'])) : 'Belum pernah diubah';
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Edit Ruangan - <?= htmlspecialchars($data['nama_ruangan']) ?> - Inventaris SDN Curug 01</title>
...
<div class="flex min-h-screen" x-data="editFormApp()" @beforeunload.window="handleBeforeUnload($event)">
        ...
        <nav class="flex-1 overflow-y-auto p-4 space-y-1">
            ...
            <a href="<?= $menu[0] ?>" 
               class="flex items-center px-4 py-3 rounded-lg text-sm transition-all duration-200 hover:bg-white/10 hover:pl-6 
                      <?= ($menu[0] == 'ruangan.php') ? 'bg-white text-primary font-semibold shadow-lg' : 'text-white/80' ?>">
                <i class="fas <?= $menu[1] ?> w-5 mr-3 text-center"></i>
                <span><?= $menu[2] ?></span>
                <?php if($menu[0] == 'ruangan.php'): ?>
    ...
    <main class="flex-1 flex flex-col min-w-0 overflow-x-hidden">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <nav class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 mb-1">
                        ...
                        <i class="fas fa-chevron-right text-[8px]"></i>
                        <span class="text-accent-orange font-semibold">Edit</span>
                    </nav>
                    <h2 class="text-xl lg:text-2xl font-bold text-accent-orange dark:text-amber-400 flex items-center gap-2">
        ...
        <div class="flex-1 p-4 lg:p-8 animate-fade-in">
            ...
            <div x-show="hasChanges" 
                 x-transition
                 class="mb-6 p-4 bg-amber-50 dark:bg-amber-900/20 border-l-4 border-amber-500 rounded-lg flex items-center gap-3">
                <i class="fas fa-exclamation-triangle text-amber-500 text-xl animate-pulse"></i>
                <div class="flex-1">
                    <p class="font-semibold text-amber-800 dark:text-amber-300">Ada Perubahan Belum Disimpan</p>
                    <p class="text-xs text-amber-700 dark:text-amber-400">Jangan lupa klik "Update" untuk menyimpan perubahan</p>
                </div>
                <button @click="resetToOriginal()" 
                        class="px-3 py-1.5 text-xs bg-amber-500 hover:bg-amber-600 text-white rounded-lg transition-all">
            ...
            <form method="POST" @submit="handleSubmit($event)" x-ref="roomForm" class="space-y-6">
                ...
                <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
                    ...
                    <div class="xl:col-span-2 space-y-6">
                        ...
                        <!-- Metadata Info Card -->
                        <div class="bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-xl border border-amber-200 dark:border-amber-800 p-4">
                            <div class="flex items-center gap-3 mb-3">
                                ...
                                <div>
                                    <h3 class="font-bold text-gray-800 dark:text-white">Informasi Data</h3>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Metadata ruangan ini</p>
                                </div>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                                <div class="bg-white dark:bg-gray-800 rounded-lg p-3">
                                    <p class="text-gray-500 dark:text-gray-400 mb-1">
                                        <i class="fas fa-hashtag mr-1"></i> ID Ruangan
                                    </p>
                                ...
                                <div class="bg-white dark:bg-gray-800 rounded-lg p-3">
                                    ...
                                    <p class="font-bold text-gray-800 dark:text-white"><?= $updated_at ?></p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="p-5 lg:p-6 space-y-5">
                                ...
                                <div>
                                    <label class="flex items-center gap-2 text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">
                                        <i class="fas fa-qrcode text-accent-orange"></i>
                                    ...
                                    <div class="relative">
                                        <input type="text" 
                                               ...
                                               @input="form.kode_ruangan = form.kode_ruangan.toUpperCase(); checkChanges()"
                                               @blur="validateKode()"
                                               required
                                               maxlength="20"
                                               :class="validation.kode.status"
                                               class="input-modern w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border rounded-lg focus:outline-none focus:border-accent-orange transition-all text-sm uppercase tracking-wider font-mono">
                                ...
                                <div>
                                    <label class="flex items-center gap-2 text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">
                                        <i class="fas fa-door-open text-accent-orange"></i>
                                    ...
                                    <input type="text" 
                                           ...
                                           required
                                           maxlength="100"
                                           class="input-modern w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg focus:outline-none focus:border-accent-orange transition-all text-sm">
                                    ...
                                    <div>
                                        ...
                                        <div class="grid grid-cols-3 gap-2">
                                            <template x-for="i in [1, 2, 3]">
                                                <button type="button" 
                                                        @click="form.lantai = i; checkChanges()"
                                                        :class="form.lantai == i ? 'bg-accent-orange text-white border-accent-orange shadow-md' : 'bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-gray-200 border-gray-200 dark:border-gray-600 hover:border-accent-orange'"
                                    ...
                                    <div class="md:col-span-2">
                                        ...
                                        <div class="relative">
                                            <input type="range" 
                                                   ...
                                                   max="100" 
                                                   step="1"
                                                   class="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-lg appearance-none cursor-pointer accent-accent-orange">
                                            <div class="flex justify-between mt-2">
                                                <span class="text-xs text-gray-400">0</span>
                                                <span class="text-sm font-bold text-accent-orange">
                            ...
                            <div class="p-5 lg:p-6 space-y-5">
                                ...
                                <div>
                                    ...
                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                                        ...
                                        <button type="button" 
                                                @click="form.fungsi_ruangan = '<?= $fungsi[0] ?>'; checkChanges()"
                                                :class="form.fungsi_ruangan === '<?= $fungsi[0] ?>' ? 'bg-blue-500 text-white border-blue-500 shadow-md' : 'bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-gray-200 border-gray-200 dark:border-gray-600 hover:border-blue-500'"
                                ...
                                <div>
                                    <label class="flex items-center justify-between mb-2">
                                        <span class="flex items-center gap-2 text-sm font-semibold text-gray-700 dark:text-gray-200">
                                            <i class="fas fa-note-sticky text-blue-500"></i>
                                            Keterangan
                                        </span>
                                    ...
                                    <textarea name="keterangan" 
                                              ...
                                              @input="if(form.keterangan.length > 250) form.keterangan = form.keterangan.substring(0, 250); checkChanges()"
                                              rows="4" 
                                              maxlength="250"
                                              placeholder="Catatan tambahan tentang ruangan ini (opsional)..."
                                              class="input-modern w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg focus:outline-none focus:border-accent-orange transition-all text-sm resize-none"></textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-100 dark:border-gray-700 p-5 lg:p-6">
                            <div class="flex flex-col sm:flex-row gap-3 justify-between">
                                ...
                                <button type="button" 
                                        @click="confirmDelete()"
                                        class="px-5 py-3 bg-red-50 dark:bg-red-900/20 hover:bg-red-500 text-red-600 hover:text-white border border-red-200 dark:border-red-800 rounded-lg transition-all flex items-center justify-center gap-2 text-sm font-medium order-2 sm:order-1">
                                ...
                                <div class="flex flex-col sm:flex-row gap-3 order-1 sm:order-2">
                                    ...
                                    <button type="submit" 
                                            ...
                                            class="px-8 py-3 bg-gradient-to-r from-accent-orange to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white rounded-lg shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2 text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed">
                            ...
                            <p class="text-xs text-gray-400 text-center mt-3">
                                ...
                                Tekan <kbd class="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-[10px]">Ctrl</kbd> + <kbd class="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-[10px]">Enter</kbd> untuk update
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="xl:col-span-1">
                        <div class="sticky top-24 space-y-4">
                            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-100 dark:border-gray-700 overflow-hidden">
                                <div class="p-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-accent-orange/5 to-transparent">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-2">
                                            <i class="fas fa-eye text-accent-orange"></i>
                                            <h3 class="font-bold text-gray-800 dark:text-white text-sm">Live Preview</h3>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <span x-show="hasChanges" 
                                                  class="text-[10px] px-2 py-0.5 bg-amber-100 text-amber-700 rounded-full font-semibold">
                                                <i class="fas fa-pen text-[8px] mr-1"></i>
                                                DIUBAH
                                            </span>
                                ...
                                <div class="p-5">
                                    <!-- Preview Card -->
                                    <div class="relative bg-gradient-to-br from-accent-orange to-amber-600 rounded-xl p-5 text-white shadow-xl overflow-hidden">
                                    ...
                                    <!-- Change Tracker -->
                                    <div class="mt-4 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-100 dark:border-amber-800">
                                        <p class="text-xs font-semibold text-amber-900 dark:text-amber-300 mb-2">
                                            <i class="fas fa-exchange-alt mr-1"></i>
                                            Perubahan Terdeteksi
                                        </p>
                                        <div class="space-y-1">
                                            <template x-for="change in changedFields">
                                                <div class="flex items-center gap-2 text-xs text-amber-700 dark:text-amber-400">
                                                    <i class="fas fa-pen text-[10px]"></i>
                                                    <span x-text="change"></span>
                                                </div>
                                            </template>
                                            <p x-show="changedFields.length === 0" class="text-xs text-gray-500 italic">
                                                Belum ada perubahan
                                            </p>
                                        </div>
                                    </div>
...
<script>
function editFormApp() {
    // Original data from database
    const originalData = {
        kode_ruangan: '<?= addslashes($data['kode_ruangan']) ?>',
        nama_ruangan: '<?= addslashes($data['nama_ruangan']) ?>',
        gedung: '<?= addslashes($data['gedung']) ?>',
        lantai: '<?= (int)$data['lantai'] ?>',
        kapasitas: <?= (int)$data['kapasitas'] ?>,
        fungsi_ruangan: '<?= addslashes($data['fungsi_ruangan']) ?>',
        penanggung_jawab: '<?= addslashes($data['penanggung_jawab']) ?>',
    ...
    return {
        ...
        get changedFields() {
            ...
            for (const key in this.form) {
                if (String(this.form[key]) !== String(this.original[key])) {
        ...
        validateKode() {
            ...
            const originalKode = this.original.kode_ruangan;
            
            if (!kode) {
                this.validation.kode = { status: 'border-red-500', message: 'Kode ruangan wajib diisi' };
                return;
            }
            if (kode.length < 3) {
            ...
            } else if (kode !== originalKode) {
                // Kode berubah - akan dicek di server apakah sudah ada
                this.validation.kode = { 
                    status: 'border-amber-500', 
                    message: 'Kode diubah - akan divalidasi saat submit' 
                };
            } else {
                this.validation.kode = { 
        ...
        resetToOriginal() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    this.hasChanges = false;
                    this.validateKode();
                    this.showToast('Perubahan direset', 'info');
                }
            });
        },
        ...
        confirmDelete() {
            Swal.fire({
                title: 'Hapus Ruangan?',
                html: `
                    <p class="text-gray-600">Anda akan menghapus ruangan:</p>
                    <p class="font-bold text-accent-orange mt-2">${this.form.nama_ruangan}</p>
                    <div class="mt-4 p-3 bg-red-50 rounded-lg text-left">
                        ...
                        <p class="text-xs text-red-600 mt-1">
                            Semua data terkait ruangan ini juga akan terhapus.
                        </p>
                    </div>
                `,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire({
                        ...
                        if (result2.isConfirmed) {
                            window.location.href = `edit_ruangan.php?id=<?= $id ?>&hapus=1`;
                        }
                    });
                }
            });
        ...
        handleSubmit(event) {
            ...
            Swal.fire({
                ...
                html: `
                    <p class="text-gray-600">Anda akan mengupdate ruangan:</p>
                    <p class="font-bold text-accent-orange mt-2">${this.form.nama_ruangan}</p>
                    ${this.changedFields.length > 0 ? `
                        <div class="mt-3 p-3 bg-amber-50 rounded-lg text-left">
                            <p class="text-xs font-semibold text-amber-900 mb-1">Field yang diubah:</p>
                            <ul class="text-xs text-amber-700 space-y-0.5">
                                ${this.changedFields.map(f => `<li>• ${f}</li>`).join('')}
                            </ul>
                        </div>
                    ` : '<p class="text-xs text-gray-500 mt-2 italic">Tidak ada perubahan terdeteksi</p>'}
                `,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#dd6b20',
                ...
                if (result.isConfirmed) {
                    ...
                    this.hasChanges = false; // Prevent beforeunload warning
                    this.$refs.roomForm.submit();
                }
            });
        },
